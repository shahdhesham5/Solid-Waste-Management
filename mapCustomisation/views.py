...
@login_required
def index(request):
    cat = Catalog("https://localhost/geoserver/rest/", username="admin", password="geoserver")
    try:
    ...
    for layer in all_layers:
        try:
            jsonLayer=(requests.get('https://localhost/geoserver/wfs?request=GetFeature&service=WFS&version=1.0.0&typeName='+layer.name+'&srsName=EPSG:4326&outputFormat=application/json').json())
        except:
            continue
        layers[layer.name]=jsonLayer
    url='https://localhost/geoserver/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=geonode:states&maxFeatures=50&outputFormat=json'
    context={
        # "layerw":that_layer,
        "layers":all_layers,
