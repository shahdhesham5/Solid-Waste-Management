...
const services_data = [
  { name: "تطبیق القاھرة التاریخیة", icon: "cairo" },
  { name: "لوحة القیادة والتحكم", icon: "dashboard" },
  { name: "إدارة أملاك الدولة", icon: "property" },
  { name: "إدارة ومراقبة الإعلانات", icon: "ads" },
  { name: "جمع وحصر البیانات", icon: "management" },
  { name: "المشاركة الجماھیریة", icon: "social" },
  { name: "نظام إدارة المخلفات", icon: "wma" },
  { name: "قاعدة بيانات", icon: "database" },
];
...
function addService(serv,index) {
  ...
  // Joe_change
  service_bubble.click(ev => serviceClick(ev, true))

  servicesList.push(service_bubble)
}
...
function createPointer() {
  ...
  service_pointer.id = 'Modulescircle'
...
function serviceClick(ev, goToUrl) {
  ...
  // Joe_change
  twn_pointer_path.tweenTo(class_index*(pointer_time/(2*6)) -.11);
  ...
  setTimeout(() => {
    if (goToUrl) location.href = services_urls[ev.currentTarget.classList[1]]
  }, 700)
}
...
// Joe_change
var pivot_path = {x: half_circle[0].x + 5, y: half_circle[0].y -80};
...
// setTimeout(()=> jQuery("#service-collection .serv-7").addClass("active"),200);
// servicesList[7].click(ev => serviceClick(ev, false))
servicesList[7].click();
